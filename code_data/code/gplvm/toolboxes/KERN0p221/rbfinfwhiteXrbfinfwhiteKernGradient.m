...
% 	rbfinfwhiteXrbfinfwhiteKernGradient.m SVN version 307
...
g1(1) = c * (invWidth2^2) * ...
    sum(sum( (sqrt((invWidth1+invWidth2)/(invWidth1*invWidth2)) ...
...
g1(2) = c * sum(sum(exp(-0.5*invWidth1*invWidth2*(deltaT.^2)/(invWidth1+invWidth2)) ...
    .* covGrad));
