...
%	Description:
%
%	KERN = KERNTEST(KERNTYPE, NUMIN, TIEPARAMNAMES) runs some tests on a
%	kernel with the specified type to ensure it is correctly
%	implemented.
%	 Returns:
%	  KERN - the kernel that was generated for the tests.
%	 Arguments:
%	  KERNTYPE - type of kernel to test. For example, 'rbf' or {'cmpnd',
%	   'rbf', 'lin', 'white'}.
%	  NUMIN - the number of input dimensions (default is 4).
%	  TIEPARAMNAMES - cell array of regular expressions for parameter
%	   names that should be tied (default is none).
%
%	KERN = KERNTEST(KERN, NUMIN, TIEPARAMNAMES) runs some tests on a
%	given kernel structure to ensure it is correctly implemented.
%	 Returns:
%	  KERN - the kernel as it was used in the tests.
%	 Arguments:
%	  KERN - kernel structure to test.
%	  NUMIN - the number of input dimensions (default is 4).
%	  TIEPARAMNAMES - cell array of regular expressions for parameter
%	   names that should be tied (default is none).
%	
...
% 	kernTest.m CVS version 1.11
% 	kernTest.m SVN version 306
